...
Config { font = "xft:Cica:size=11"				-- font (xftってのがフォントにアクセスしてる)
		...
		, lowerOnStart = True						-- if true bar lower other windows (クソ雑魚英語)
		, hideOnStart = False						-- bar is hidden when to start (クソ雑魚英語)
		, allDesktops = True						-- all desktop use bar
		, overrideRedirect = True					-- 普通のwindow扱いしないならTrue
		, pickBroadest = False						-- クソデカモニター君にbar を出すなら True
